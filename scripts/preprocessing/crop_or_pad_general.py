...
def preprocess(path_dir):
    ...
    for n, path_img in enumerate(path_dir.glob('sub.nii.gz')):
        # Read image
...
if __name__ == "__main__":
    path_data = Path('/home/jeff/Desktop/data_combridge/nifty')
    path_out = Path('/home/jeff/Desktop/data_combridge/cropped/')
    path_out.mkdir(parents=True, exist_ok=True)
    pool = Pool()
    print(list(path_data.glob('*')))
    pool.map(preprocess, path_data.glob('*/'))  # change this line    # for path_dir in path_data.iterdir():
    #     preprocess(path_dir)
